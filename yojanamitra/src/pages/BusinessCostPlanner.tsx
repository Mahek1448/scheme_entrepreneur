...
const DEFAULT_ITEMS: CostItem[] = [
  { id: '1', category: 'Equipment', name: 'Gas Stove & Cylinders', estimatedCost: 8000, isEssential: true },
  { id: '2', category: 'Equipment', name: 'Kettle, Cups & Utensils', estimatedCost: 3500, isEssential: true },
  { id: '3', category: 'Setup', name: 'Cart / Stall Setup & Branding', estimatedCost: 15000, isEssential: true },
  { id: '4', category: 'Raw Materials', name: 'Tea, Milk, Sugar (1 month)', estimatedCost: 7000, isEssential: true },
  { id: '5', category: 'Licenses', name: 'FSSAI Food License & Local Permits', estimatedCost: 2500, isEssential: true },
  { id: '6', category: 'Working Capital', name: 'Operating Cash (2 months)', estimatedCost: 10000, isEssential: true },
  { id: '7', category: 'Marketing', name: 'Signboard & Menu Board', estimatedCost: 1500, isEssential: false },
  { id: '8', category: 'Contingency', name: 'Buffer / Unexpected Expenses', estimatedCost: 5000, isEssential: false },
];
      ...
      {showDetail && (
        <>
          ...
          <p className="text-xs text-gray-400 mt-3 bg-gray-50 rounded-lg p-2">
            ⚠ This is an illustration using standard reducing-balance formula. Interest rate entered above is for demonstration only — actual rates depend on the lender and your credit profile.
          </p>
        </>
      )}
    </div>
...
function SupportStack({ capital, loan, subsidy, total, language }: {
  ...
  const bars = [
    ...
    { label: 'Loan / Credit', value: loan, pct: loanPct, color: 'bg-blue-500', textColor: 'text-blue-600', description: 'e.g. MUDRA Shishu' },
  ...
  return (
    <div className="card border-2 border-green-100">
      ...
      <div className="mt-3 p-2.5 bg-blue-50 rounded-lg flex items-start gap-2 text-xs text-blue-700">
        ...
        Subsidy is calculated from PMEGP scheme data (25% of project cost for OBC/urban). Loan is the funding gap after your capital and subsidy. These are illustrative — actual amounts depend on bank approval.
      </div>
    </div>
  );
}
...
export default function BusinessPlanner() {
  ...
  // Calculate potential subsidy from PMEGP (25% for OBC/urban) — from real scheme data
    ...
    <div className="page-container py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {/* EMI Calculator */}
          <EMICalculator loanAmount={fundingGap > 0 ? fundingGap : 50000} language={language} />
        </div>
          ...
          <div className="card border-2 border-[#1e3a5f]/15">
            ...
            <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-1">
              <div
                ...
                style={{ width: `${Math.min(100, Math.round((capital / total) * 100))}%` }}
              />
            </div>
            <p className="text-xs text-gray-400 text-right">{Math.min(100, Math.round((capital / total) * 100))}% self-funded</p>

            {!fullyFunded && (
              <div className="mt-3 bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-700">
                A <strong>MUDRA Shishu</strong> loan of up to ₹50,000 could cover your gap.
              </div>
            )}
      ...
      <div className="mt-6">
        <SupportStack
          capital={capital}
          loan={Math.min(fundingGap, 50000)}
